  ...
  function dimension(value, iterable) {
    ...
    function preAdd(newData, n0, n1) {
      ...
      if (iterable) {
        ...
        iterablesIndexCount = xfilterArray.arrayLengthen(
          iterablesIndexCount,
          n
        );
      ...
      for (; i0 < n0 && i1 < n1; ++index5) {
        if (oldValues[i0] < newValues[i1]) {
          ...
          if (iterable)
            iterablesIndexFilterStatus[index5] =
              oldIterablesIndexFilterStatus[i0];
          index[index5] = oldIndex[i0++];
        } else {
          values[index5] = newValues[i1];
          if (iterable)
            iterablesIndexFilterStatus[index5] =
              newIterablesIndexFilterStatus[i1];
    ...
    function filterExact(value) {
      ...
      return filterIndexBounds(
        (refilter = xfilterFilter.filterExact(bisect, value))(values)
      );
    }
    ...
    function filterRange(range) {
      ...
      filterValuePresent = true;
      return filterIndexBounds(
        (refilter = xfilterFilter.filterRange(bisect, range))(values)
      );
    }
    ...
    function filterIndexFunction(f, filterAll) {
      ...
      if (!iterable) {
        for (i = 0; i < indexLength; ++i) {
          if (
            !(filters[offset][(k = index[i])] & one) ^ !!(x = f(values[i], i))
          ) {
            if (x) added.push(k);
            else removed.push(k);
          }
      ...
      if (!iterable) {
        for (i = 0; i < added.length; i++) {
          if (filters[offset][added[i]] & one)
            filters[offset][added[i]] &= zero;
        }

        for (i = 0; i < removed.length; i++) {
          if (!(filters[offset][removed[i]] & one))
            filters[offset][removed[i]] |= one;
        }
      } else {
        var newAdded = [];
    ...
    function group(key) {
      ...
      function add(newValues, newIndex, n0, n1) {
        ...
        if (iterable) {
          ...
          groupIndex =
            k0 > 1
              ? xfilterArray.arrayLengthen(groupIndex, n)
              : cr_index(n, groupCapacity);
        }
        ...
        while (i1 < n1) {
          ...
          while (x1 <= x) {
            ...
            if (!filters.zeroExcept(j, offset, zero))
              g.value = remove(g.value, data[j], false);
            if (++i1 >= n1) break;
            x1 = key(newValues[i1]);
          }
